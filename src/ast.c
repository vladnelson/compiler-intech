...
{
  ast_t *astInteger = malloc(sizeof(ast_t));
  astInteger->type = AST_INTEGER;
  astInteger->integer = val;
  return astInteger;
}

ast_t *ast_new_binary(ast_binary_e op, ast_t *left, ast_t *right)
{
  ast_t *astBinary = malloc(sizeof(ast_t));
  astBinary->type = AST_BINARY;
  astBinary->binary.op = op;
  astBinary->binary.left = left;
  astBinary->binary.right = right;
  return astBinary;
}
...
static int ast_print_(ast_t *ast, size_t indent)
{
  if (!ast)
  {
    printf("x");
